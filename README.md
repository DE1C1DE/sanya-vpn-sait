...
Поддерживаются Debian 12/13 и Ubuntu с `apt`, `systemd` и nginx. Нужен сервер с открытыми TCP-портами `80` и `443`, а DNS A-запись домена должна указывать на его публичный IP.
...
```bash
sudo bash install.sh --domain vpn-ot-sani.work.gd --admin-password 'сложный-пароль'
```
